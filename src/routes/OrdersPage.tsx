...
const statusSteps = ['PENDING', 'PAID', 'PREPARING', 'SHIPPED', 'DELIVERED'] as const;
  ...
  return (
    <main className="min-h-screen bg-[#f7f9fb] px-4 md:px-10 py-8">
      <div className="max-w-6xl mx-auto">
        <button type="button" onClick={() => navigate(-1)} className="mb-5 inline-flex items-center gap-2 text-sm font-semibold text-blue-700 hover:underline">← Back</button>
        ...
        {orders.length === 0 ? (
          ...
          const current = order.status === 'CANCELLED' ? -1 : statusSteps.indexOf(order.status as typeof statusSteps[number]);
          return <article key={order.id} className="bg-white border border-slate-200 rounded-2xl p-5 md:p-6 shadow-sm">
            ...
            <div className="py-6">{order.status === 'CANCELLED' ? <p className="text-sm font-semibold text-red-600">This order has been cancelled.</p> : <div className="grid grid-cols-5 gap-2">{statusSteps.map((step, index) => <div key={step} className="text-center"><div className={`h-2 rounded-full mb-2 ${index <= current ? 'bg-blue-600' : 'bg-slate-200'}`}/><span className={`text-[11px] font-semibold ${index <= current ? 'text-blue-700' : 'text-slate-400'}`}>{step}</span></div>)}</div>}</div>
